...
const sidebarNavItems = [
  ...
  {
    id: 2,
    label: "Configure Clone",
    icon: "MinusCircleOutlined",
    link: "/CloneInteraction",
  },
...
const Sidebar = () => {
  return (
    <div className="fixed text-white flex flex-col justify-between h-screen w-70 sm:w-20 md:w-60 p-2 sm:p-2 md:p-4 gap-6 md:gap-8 z-50">
      {/* Logo Section */}
        ...
        <ul className="flex flex-col gap-2 md:gap-4">
          {sidebarNavItems.map((item) => {
            ...
            return (
              <li key={item.id}>
                <Link to={item.link}>
                  <div className="flex items-center gap-2 py-2 px-4 md:py-4 md:px-6 border border-transparent hover:border-spacing-1 hover:border-current rounded-full cursor-pointer box-border">
